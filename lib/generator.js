...
module.exports = function fixtureGenerator( options, cb ) {
  ...
  function extractFixturesData( data ) {
    return Promise.map( data.section(), function( section ) {
      var ext = path.extname( section.markup() ),
          testDir = "";

      data.data.files[0].split("/").forEach(function(dir, i){
        if(i <= 2){
          testDir += dir + "/";
        }
      })
      testDir += path.dirname( section.markup() ) + "/" + path.basename( section.markup(), ext ) + ext;
