...
module.exports = function fixtureGenerator( options, cb ) {
  ...
  if ( !options.stylesheets || {}.toString.call( options.stylesheets ) !== "[object Array]" ) {
    throw new Error( "You must provide an array of files/directories of compiled stylesheets to include." );
    return;
  }
  ...
  function extractFixturesData( data ) {
    return Promise.map( data.section(), function( section ) {
      var testDir = data.data.files[0].split("/")[0] + "/" + data.data.files[0].split("/")[1] + "/" + data.data.files[0].split("/")[2] + "/" + path.dirname( section.markup() )
      testDir += "/" + path.basename( section.markup(), path.extname( section.markup() ) ) + path.extname( section.markup() )

      return Promise.props({
        header : section.header(),
        description : section.description(),
        markup : fs.readFileAsync( testDir, 'utf8' ).then( console.log ),
